...
{
    public class EditNumCommand : IHotKeyCommand
    {
            ...
            get
            {
                return new Action<Window>((win) =>
                {
                    ...
                    if (dg != null && dg.CurrentGrid != null && dg.CurrentGrid.HasItems)
                    {
                        ...
                        {
                            items.ElementAt(dg.CurrentGrid.SelectedIndex).EditNumCommand.Execute(null);
                        }
                        else if (items.Count() > 0)
                        {
                            items.LastOrDefault().EditNumCommand.Execute(null);
                        }
                    }
                });
            }
