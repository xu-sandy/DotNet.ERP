...
{
    public class SalesRecordSyncLocalService : ISyncDataService, ILocalDescribe
    {
        ...
        public IEnumerable<ISyncDataObject> GetSyncObjects(int companyId, string storeId)
        {
            try
            {
                ...
                {
                    var result = db.SalesRecords.Select(o => new SyncDataObject() { SyncItemId = o.SyncItemId, SyncItemVersion = o.SyncItemVersion }).ToList();
                    return result;
                }
            }
            catch
        ...
        public byte[] CreateItem(ISyncDataObject data, Guid guid, int companyId, string storeId)
        {
            ...
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var entity = new Pharos.Logic.ApiData.Pos.Entity.LocalCeEntity.SalesRecord();
