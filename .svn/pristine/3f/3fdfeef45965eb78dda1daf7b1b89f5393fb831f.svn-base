...
namespace Pharos.CMS.Retailing.Controllers
{
    public class BaseController : Controller
    {
        ...
        /// <returns></returns>
        protected ActionResult RedirectAlert(string actionName,string msg, bool isTop=false)
        {
            return Content(string.Format("<script>alert('{0}');{1}window.location.href='", msg, isTop ? "top." : "") + Url.Action(actionName) + "'</script>");
        }
    }
}
