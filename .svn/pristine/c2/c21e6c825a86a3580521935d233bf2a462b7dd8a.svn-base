    ...
    public class Authorize
    {
        ...
        static OpResult ValidateProperty(CompanyAuthorize source)
        {
            ...
            if (source.Way == 1)
            {
                ...
                {
                    RemoveCurrentAuth();
                    return OpResult.Fail("连接OMS管理平台失败，请检查网络是否正常!","404");
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public static CompanyAuthorize GetCompanyByConnect(ref OpResult op)
        {
            var comp = Pharos.Sys.CurrentUser.Company as CompanyAuthorize;
            if (comp == null)
            {
                for (int i = 0; i < 3; i++)
                {
                ...
                comp = Pharos.Sys.CurrentUser.Company as CompanyAuthorize;
            }
            return comp;
        }
        ...
        {
            if (OmsUrl.IsNullOrEmpty()) return OpResult.Success();//临时
