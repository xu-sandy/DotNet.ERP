...
namespace Pharos.Sys.DAL
{
    ...
    internal class SysDataDictionaryDAL : BaseSysEntityDAL<SysDataDictionary>
    {
        ...
        internal DataTable GetDicListByPSN(int psn)
        {
            ...
            sql.Append("WHERE CompanyId=@companyId and DicPSN=@DicPSN AND [STATUS]=1; ");
        ...
        internal SysDataDictionaryExt GetExtModelByDicSN(int dicSN)
        {
            ...
            sql.Append("SELECT a.*,b.Title AS PTitle FROM dbo.SysDataDictionary a ");
            sql.Append("LEFT JOIN dbo.SysDataDictionary b ON b.DicSN=a.DicPSN ");
            sql.Append("WHERE a.DicSN=@DicSN and a.CompanyId=@CompanyId");
        ...
        /// <returns></returns>
        public bool ExistsTitle(int sn, int psn, string title,int companyId)
        {
            SqlParameter[] parms = {
            ...
            string sql = string.Format("select count(1) from {0} where [DicSN]<>@sn and DicPSN=@psn and Title=@title and companyId=@companyId", TableName);
