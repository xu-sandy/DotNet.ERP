...
namespace Pharos.SyncService
{
    public class PosDbSyncProvider : KnowledgeSyncProvider, IChangeDataRetriever, INotifyingChangeApplierTarget
    {
        ...
        public void WriteLog(string msg)
        {
            if (ReadRedisLog && _syncContext is ILocalDescribe)
            {
                StoreManager.SetSyncLog(_syncContext.Name + "SyncLogCache", string.Format("<tr><td>{0} <span class=\"wrong\">{1}</span></td></tr>", (_syncContext as ILocalDescribe).Describe, msg));
            }
        }
        ...
        public void EndSession()
        {
            ...
            if (ReadRedisLog && _syncContext is ILocalDescribe)
            {
                StoreManager.SetSyncLog(_syncContext.Name + "SyncLogCache", string.Format("<tr><td>{0} 同步<span class=\"finish\">已完成</span>，本次同步时间{1}。</td></tr>", (_syncContext as ILocalDescribe).Describe, DateTime.Now.ToString("dd日 HH:mm:ss")));
            }
        }
