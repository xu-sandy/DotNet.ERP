...
{
    public class NoticeSyncLocalService : ISyncDataService, ILocalDescribe
    {
        ...
        public IEnumerable<ISyncDataObject> GetSyncObjects(int companyId, string storeId)
        {
            try
            {
                ...
                {
                    var result = db.Notices.Select(o => new SyncDataObject() { SyncItemId = o.SyncItemId, SyncItemVersion = o.SyncItemVersion }).ToList();
                    return result;
                }
            }
            catch
        ...
        public ISyncDataObject GetItem(Guid guid, int companyId, string storeId)
        {
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var entity = db.Notices.Where(o => o.SyncItemId == guid).FirstOrDefault();
                if (entity != null)
        ...
        public byte[] CreateItem(ISyncDataObject data, Guid guid, int companyId, string storeId)
        {
            ...
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var entity = new Pharos.Logic.ApiData.Pos.Entity.LocalCeEntity.Notice();
        ...
        public byte[] UpdateItem(Guid guid, ISyncDataObject mergedData, int companyId, string storeId)
        {
            ...
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var dbEntity = db.Notices.FirstOrDefault(o => o.SyncItemId == guid);
        ...
        public void DeleteItem(Guid syncItemId, int companyId, string storeId)
        {
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var dbEntity = db.Notices.FirstOrDefault(o => o.SyncItemId == syncItemId);
