...
namespace Pharos.Logic.BLL
{
    ...
    public class RechargeGiftsService : BaseService<RechargeGifts>
    {
        ...
        /// <returns></returns>
        public object GetRechargeGiftInfo(decimal rechargeMoney, short category)
        {
            List<RechargeGifts> rules = new List<RechargeGifts>();
            //查询条件》充值金额的规则  活动状态=活动中  在当前时间内
            var query = CurrentRepository.QueryEntity.Where(o => o.ConditionValue < rechargeMoney && o.CompanyId == CommonService.CompanyId && o.State == 1 && o.Category == category).OrderByDescending(o => o.CreateDT).ToList();
            ...
            foreach (var item in query)
            {
                ...
                {
                    var count = BaseService<MemberRecharge>.CurrentRepository.QueryEntity.Where(o => o.RechargeGiftId == item.RuleId).Sum(o => o.Number);
                    if (count >= item.LimitNumber)
                    {//超出限量，本条规则不统计
                        continue;
                    }
                }
