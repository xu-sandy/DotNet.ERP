...
namespace Pharos.Logic.BLL
{
    public class InventoryRecordService : BaseService<InventoryRecord>
    {
            ...
            foreach(var rd in records)
            {
                ...
                else if (info.Nature == 2 && outSources.Contains(rd.Source))//出库存拆分
                {
                    ...
                    if (childProductInventory != null)
                    {
                        ...
                        {
                            var ir = new InventoryRecord() { Number = stockNum, Barcode = rd.Barcode, StoreId = storeId, Source = rd.Source, OperatId = rd.OperatId, CreateDT = rd.CreateDT, CreateUID = rd.CreateUID };
        ...
        static int[] outSources = new int[] { 2, 4, 6, 8, 9, 10, 12, 13, 14,16 };//减库存
        ...
        static void SplitSingleProduct(decimal purchaseNumber,string storeId, ProductRecord product,ref List<InventoryRecord> irs)
        {
            ...
            var ir = new InventoryRecord() { StoreId = storeId, Barcode = product.OldBarcode, Number = oldProductNum, Source = 4 ,CreateDT=DateTime.Now,CreateUID=Sys.CurrentUser.UID};
            UpdateInvent(inventList, ir);
            irs.Add(ir);
