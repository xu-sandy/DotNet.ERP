...
namespace Pharos.Logic.BLL
{
    public class SaleOrdersService : BaseService<SaleOrders>
    {
        ...
        /// </summary>
        /// <returns></returns>
        public static List<SelectListItem> GetCashiers(string storeId = null)
        {
            var query = CurrentRepository.Entities.Where(o => o.CompanyId == CommonService.CompanyId);
            if (!storeId.IsNullOrEmpty()) query = query.Where(o => o.StoreId == storeId);
            var uids = query.Select(o => o.CreateUID).Distinct().ToList();
        ...
        /// </summary>
        /// <returns></returns>
        public static List<SelectListItem> GetSalers(string storeId = null)
        {
            var query = CurrentRepository.Entities.Where(o => o.CompanyId == CommonService.CompanyId);
            if (!storeId.IsNullOrEmpty()) query = query.Where(o => o.StoreId == storeId);
            var uids = query.Select(o => o.Salesman).Where(o => o != null || o != "").Distinct().ToList();
            var list = UserInfoService.FindList(o => uids.Contains(o.UID));
            return list.Select(o => new SelectListItem()
            {
                Text = o.FullName,
        ...
        {
            var query = CurrentRepository.QueryEntity.Where(o => o.CreateDT >= beginTime && o.CreateDT <= endTime && !o.IsTest && o.State == 0 && o.Type == 0 && o.CompanyId == CommonService.CompanyId);
        ...
        public static object GetDetailBypaysn(string paysn)
        {
            ...
            var query = from a in CurrentRepository.QueryEntity
                        ...
                        select new
                        {
                            ...
                            TradeNo = e.TradeNo,
                            Member = (a.MemberId == null || a.MemberId == "") ? "" : MembersService.CurrentRepository.QueryEntity.Where(o => o.MemberId == a.MemberId).Select(o => (o.RealName == null || o.RealName == "") ? o.MobilePhone : o.RealName + "(" + o.MobilePhone + ")").FirstOrDefault()
                        };
            ...
            }).Select(o => new
            {
                o.Key.PaySN,
                o.Key.CustomOrderSn,
                o.Key.ReFundOrderCustomOrderSn,
                o.Key.Type,
                o.Key.CreateUID,
                o.Key.TypeTitle,
                o.Key.Salesman,
                ...
                OrderDiscount = (-Math.Abs(o.Key.OrderDiscount)).ToAutoString(),
            }).FirstOrDefault();
            return obj;
        }
        ...
        public static System.Data.DataTable QuerySaleOrdersPageList(NameValueCollection nvl, out int recordCount, out object footer)
        {
            ...
            decimal PurchaseNumberTotal = 0, AmountTotal = 0, PreferentialPriceTotal = 0, OrderDiscount = 0;
            if (dt.Rows.Count > 0)
            {
                PurchaseNumberTotal = dt.Rows[0]["ProductCounts"].ToType<decimal>();
                AmountTotal = dt.Rows[0]["TotalAmounts"].ToType<decimal>();
                PreferentialPriceTotal = dt.Rows[0]["PreferentialPrices"].ToType<decimal>();
                OrderDiscount = dt.Rows[0]["OrderDiscounts"].ToType<decimal>();
            }
            footer = new List<object>() { 
                new {PurchaseNumber=PurchaseNumberTotal,TotalAmount=AmountTotal,PreferentialPrice=PreferentialPriceTotal,OrderDiscount=OrderDiscount,Title="合计:"}
            };
            return dt;
        }
