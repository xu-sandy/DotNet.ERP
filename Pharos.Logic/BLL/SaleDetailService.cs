...
namespace Pharos.Logic.BLL
{
    public class SaleDetailService : BaseService<SaleDetail>
    {
        ...
        /// <param name="footer"></param>
        /// <returns></returns>
        public static object GetConsumptionPayments(string paySN)
        {
            var query = from o in BaseService<ConsumptionPayment>.CurrentRepository.Entities
                        where o.CompanyId == CommonService.CompanyId && o.PaySN == paySN
                        select new
                        {
                            o.Amount,
                            ...
                            o.Change,
                            o.Received,
                            ApiTitle=BaseService<ApiLibrary>.CurrentRepository.QueryEntity.Where(i=>i.CompanyId==o.CompanyId && i.ApiCode==o.ApiCode).Select(i=>i.Title).FirstOrDefault(),
                            TradeNo = BaseService<PayNotifyResult>.CurrentRepository.QueryEntity.Where(i => i.CompanyId == o.CompanyId && i.PaySN == o.ApiOrderSN).Select(i => i.TradeNo).FirstOrDefault()??o.ApiOrderSN
                        };
            var list = query.ToList();
            return list;
        }
