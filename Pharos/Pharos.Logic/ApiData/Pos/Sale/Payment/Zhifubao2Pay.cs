...
{
    public class Zhifubao2Pay : BasePay, IThirdPartyPaymentWithoutWait
    {
        public Zhifubao2Pay()
            : base(17, PayMode.ScanZhifubao)
        {
        }
        public object RequestPay()
        {
            var request = (HttpWebRequest)WebRequest.Create(System.IO.Path.Combine(ApiUrl, "Qrcode/MicroPay"));

            string msg = string.Format("storeId={2}&paySN={0}&totalPrice={1:f2}&autoCode={3}&CompanyToken={4}", base.PayDetails.PaySn, base.PayDetails.Amount, base.PayDetails.StoreId, base.PayDetails.CardNo, base.PayDetails.CompanyToken);
            request.Method = "POST";
