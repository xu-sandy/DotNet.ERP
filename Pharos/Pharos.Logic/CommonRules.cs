    ...
    public static class CommonRules
    {
        ...
        public static string OrderSN
        {
            get
            {
                lock (objlock)
                {
                    var prefix = DateTime.Now.ToString("yyyyMMddHH");
                    var maxCode = BLL.BaseService<Entity.IndentOrder>.CurrentRepository.QueryEntity.Where(o => o.IndentOrderId.StartsWith(prefix)).Max(o => o.IndentOrderId);
                    int code = 0;
                    if (!maxCode.IsNullOrEmpty())
                        code = int.Parse(maxCode.Substring(10));
                    code++;
                    return prefix + code.ToString("0000");
                }
            }
        }
        /// <summary>
        ...
        /// 生成新的会员号
        /// </summary>
        public static string MemberNum(string storeId)
        {
            //规则：1位边界+7位商户号+4位门店+6位序号
            var no = "1" + Pharos.Logic.BLL.CommonService.CompanyId.ToString().PadLeft(7, '0') +storeId.PadLeft(4,'0');
            var maxCode = BLL.BaseService<Entity.Members>.CurrentRepository.QueryEntity.Where(o =>o.MemberNo.Length==18 && o.MemberNo.StartsWith(no)).Max(o => o.MemberNo);
            int code = 0;
            if (!maxCode.IsNullOrEmpty())
                code = int.Parse(maxCode.Substring(12));
            code++;
            return no + code.ToString().PadLeft(6,'0');
        }
        /// <summary>
        /// 促销时段
        /// </summary>
