...
(function (para) {
    para.manager = {
        ...
        viewurlNocache: function () {
            var url = this.viewurl ? this.viewurl : this.editurl;
            url = url.indexOf("?") == -1 ? url + "?" : url + "&";
            url += "t=" + Math.random()+"&view=1";
            if (this.Id) url += "&Id=" + this.Id;
            return url;
        },
                ...
                onClickCell: function (index, field, value) {
                    ...
                    if (editBefore(rowData, field, index, value)) {
                        if (editNo()) {
                            ...
                            para.manager.viewInfo(rowData.Id, rowData);
                        }
                    }
                }
            });
        ...
        viewInfo: function (Id, row) {
            this.Id = Id;
            openDialog600(this.viewText, this.viewurlNocache(), false, true);
        },
...
$(function () {
    ...
    if (pharos.manager.columns.length > 0) {
        ...
        if (width < $(window).width()) {//如果总宽度比屏幕小，则自动拉长列
            pharos.manager.$dg.datagrid({fitColumns:true});
        }
    }
});
