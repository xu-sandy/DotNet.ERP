...
namespace Pharos.Logic.BLL
{
    public class IntegralRuleService : BaseService<IntegralRule>
    {
        ...
        public object FindIntegralRuleList(short ruleTypeId, string value, out int count)
        {
            ...
            if (ruleTypeId != -1)
            {
                query = query.Where(o => o.RuleTypeId == ruleTypeId);
            }
            count = query.Count();
            if (count > 0)
            {
                ...
                foreach (var item in _list)
                {
                    switch (item.RuleTypeId)
                    {
                        ...
                        case 3:
                            ...
                            var category = BaseService<ProductCategory>.CurrentRepository.Entities.Where(o => ("," + item.RuleTypeValue + ",").Contains("," + o.CategorySN + ",")).ToList();
                            if (category != null)
                            {
                                item.RuleTypeValue = "";
        ...
        {
            return CurrentRepository.Entities.Where(o => o.CompanyId == CommonService.CompanyId && o.State == 0 && o.UseUsers == memberLevel);
        }
    }
}
