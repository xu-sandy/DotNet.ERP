...
{
    public class AreaSyncLocalService : ISyncDataService, ILocalDescribe
    {
        ...
        {
            var temp = mergedData as Pharos.SyncService.SyncEntities.Area;
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var dbEntity = db.Areas.FirstOrDefault(o => o.SyncItemId == guid);
                dbEntity.InitEntity(temp);
                db.SaveChanges();
                var newEntity = db.Areas.FirstOrDefault(o => o.SyncItemId == guid);
