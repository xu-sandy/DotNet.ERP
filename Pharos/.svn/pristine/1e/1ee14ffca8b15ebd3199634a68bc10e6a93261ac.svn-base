...
namespace Pharos.Logic.ApiData.Pos.DataAdapter
{
    ...
    public class SqlServerDataAdapter : IDataAdapter
    {
        ...
        public StoredValueCardRechargeResult StoredValueCardRecharge(int type, string cardNo, decimal amount)
        {
            ...
            var result = new StoredValueCardRechargeResult() { Amount = amount, StateInfo = cardState, balance = membershipCard.Balance, CardNo = cardNo, Name = member == null ? "" : member.RealName };
            if (amount > 0)
            {
