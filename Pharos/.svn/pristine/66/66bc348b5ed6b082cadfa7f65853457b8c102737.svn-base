...
namespace Pharos.Infrastructure.Data.Normalize
{
    ...
    public class AppPayOrderSn : SwiftNumber
    {
        public AppPayOrderSn(int companyId, string storeId)
            : base(string.Format("AppPayOrderSn{0}_{1}", companyId, storeId))
        {
            CompanyId = companyId;
            StoreId = storeId;
        }
        ...
        public override string ToString()
        {
            string result = string.Format("P{0}{1:yyyyMMdd}{2:000000}", StoreId.PadLeft(5, '0'), DateTime.Now, GetNumber());
            Next();
            return result;
        }
