...
{
    public class MembershipCardSyncLocalService : ISyncDataService, ILocalDescribe
    {
        ...
        public IEnumerable<ISyncDataObject> GetSyncObjects(int companyId, string storeId)
        {
            try
            {
                using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
                {
                    ...
                    var result = db.MembershipCards.Select(o => new SyncDataObject() { SyncItemId = o.SyncItemId, SyncItemVersion = o.SyncItemVersion }).ToList();
                    return result;
                }
            }
            catch
        ...
        public ISyncDataObject GetItem(Guid guid, int companyId, string storeId)
        {
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var entity = db.MembershipCards.Where(o => o.SyncItemId == guid).FirstOrDefault();
                if (entity != null)
        ...
        {
            var temp = data as Pharos.SyncService.SyncEntities.MembershipCard;
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var entity = new Pharos.Logic.ApiData.Pos.Entity.LocalCeEntity.MembershipCard();
                entity.InitEntity(temp);
                db.MembershipCards.Add(entity);
                db.SaveChanges();
        ...
        {
            var temp = mergedData as Pharos.SyncService.SyncEntities.MembershipCard;
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var dbEntity = db.MembershipCards.FirstOrDefault(o => o.SyncItemId == guid);
                dbEntity.InitEntity(temp);
                db.SaveChanges();
                var newEntity = db.MembershipCards.FirstOrDefault(o => o.SyncItemId == guid);
        ...
        public void DeleteItem(Guid syncItemId, int companyId, string storeId)
        {
            using (var db = SyncDbContextFactory.Factory<LocalCeDbContext>())
            {
                var dbEntity = db.MembershipCards.FirstOrDefault(o => o.SyncItemId == syncItemId);
                db.MembershipCards.Remove(dbEntity);
                db.SaveChanges();
            }
        }
