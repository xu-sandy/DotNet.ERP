    ...
    public class ProductRoleVerService
    {
        ...
        public OpResult Copy(string verId)
        {
            ...
            if (pro != null && pro.Status != 0)
            {
                ...
                #region 如果发现有新版本，则取新的配置并赋值前所选
                var modelVer= ProductModelVerService.GetOfficialLast(pro.ProductId);
                if(modelVer!=null && modelVer.ModuleId!=pro.ModuleId)
                {
                    obj.ProductRoles.Each(role =>
                    {
                        var roleDatas = new List<ProductRoleData>();
                        var limits= modelVer.ProductMenuLimits.Where(o => o.Type == 3);
                        foreach (var ml in modelVer.ProductMenuLimits.Where(o => o.Type != 3))
                        {
                            var data = role.ProductRoleDatas.FirstOrDefault(o => o.MenuId == ml.MenuId);
                            var lids = new List<int>();
                            var selected = false;
                            if (data != null)
                            {
                                selected = data.HasSelected;
                            }
                            roleDatas.Add(new ProductRoleData()
                            {
                                PMenuId = ml.PMenuId,
                                MenuId = ml.MenuId,
                        ...
                        role.ProductRoleDatas = roleDatas;
                    });
                    obj.ModuleId = modelVer.ModuleId;
                }
                #endregion
