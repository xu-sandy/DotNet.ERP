...
namespace Pharos.Logic.OMS.BLL
{
    public class SysRoleService
    {
        ...
        public List<SysRoles> GetRolesByDept(int deptId,bool all=true)
        {
            if(!all)
                return RoleRepository.GetQuery(o => o.Status && (("," + o.DeptId + ",").Contains("," + deptId + ",") || o.DeptId==null || o.DeptId=="")).ToList();
            else
                return RoleRepository.GetQuery(o => ("," + o.DeptId + ",").Contains("," + deptId + ",") || o.DeptId == null || o.DeptId == "").ToList();
        }
        public void SetState(short state, string id)
        {
            ...
            list.Each(o => {
                o.Status = Convert.ToBoolean(state);
            });
            RoleRepository.SaveChanges();
        }
        ...
        public List<SysRoles> GetSysRolesList()
        {
            ...
            {
                SysUser user = UserRepository.GetQuery(o => o.UserId == CurrentUser.UID).FirstOrDefault();
                if (user != null)
                {
                    if (!user.RoleIds.IsNullOrEmpty())
                    {
                        var RoleIds = user.RoleIds.Split(',').Select(o=>int.Parse(o));
                        list = RoleRepository.GetQuery(o=>RoleIds.Contains(o.RoleId)).ToList();
                    }
                }
            }
            return list;
