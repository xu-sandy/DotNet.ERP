...
namespace Pharos.OMS.Retailing.Controllers
{
    public class AgentsInfoController : BaseController
    {
        ...
        public ActionResult Index()
        {
            ...
            //代理商类型
            ViewBag.ty = ListToSelect(tradersService.getDataList(457).Select(o => new SelectListItem() { Value = o.DicSN.ToString(), Text = o.Title }), emptyTitle: "全部");
            return View();
        }
        ...
        public ActionResult Save(int? id)
        {
            ...
            {
                //初始化
                OpResult op = agentsInfoService.InitializeSave(Convert.ToInt32(id));
                if (!op.Successed)
                {
                    return Content("发生错误！");
                }
                ...
                if (!obj.IdCardPhoto.IsNullOrEmpty())
                {
            ...
            //是否存在上级代理商（0不存在，1是存在）
            ViewBag.isPAgentsId = isPAgentsId;
            //证件照
            ViewBag.img = img;
            //结算账户
            ...
            //交易支付通道
            ViewBag.PayC = agentsInfoService.GetPayApiList().CreateSelect("Title", "ApiNo", "请选择", "", agentPay.ApiNo.ToString());
            //指派人
