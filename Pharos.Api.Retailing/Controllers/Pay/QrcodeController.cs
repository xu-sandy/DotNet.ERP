...
namespace Pharos.Api.Retailing.Controllers.Pay
{
    public class QrcodeController : Controller
    {
        ...
        #region 扫码即时到帐
        public ActionResult QrcodePay(int? CompanyId, string storeId, string paySN, decimal totalPrice)
        {
            var op = new OpResult();
            ...
            else
            {
                ...
                string biz_type = "10";
                //json数据
                string biz_data = GetBizData(CompanyId.GetValueOrDefault(), paySN, totalPrice);
                var sHtmlText = "";
                try
                {
        ...
        /// <returns></returns>
        private string GetBizData(int CompanyId, string OrderNo, decimal price)
        {
            ...
            string goods = "\"goods_info\":{\"id\":\"" + OrderNo + "\",\"name\":\"订单编号" + CompanyId + "_" + OrderNo + "\",\"price\":\"" + price + "\",\"expiry_date\":\"" + DateTime.Now.ToString("yyyy-MM-dd HH:mm:ss") + "|" + DateTime.Now.AddMinutes(10).ToString("yyyy-MM-dd HH:mm:ss") + "\"}";
            sbJson.Append(goods);
            sbJson.Append("}");
        ...
        public int GetStatus(int apiCode, int companyId, string paySn)
        {
            var entity = Pharos.Logic.BLL.PayNotifyResultService.Find(o => o.ApiCode == apiCode && o.PaySN == paySn && o.CompanyId == companyId);
            if (entity == null)
            {
                return 0;
            }
