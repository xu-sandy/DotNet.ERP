...
namespace Pharos.Store.Retailing.Controllers
{
    public class BaseController : AsyncController
    {
        ...
        /// <param name="selectValue"></param>
        /// <returns></returns>
        protected IList<SelectListItem> ListToSelect(IEnumerable<SelectListItem> ies, string emptyTitle = null, string emptyValue = "", object selectValue = null)
        {
        ...
        {
            var url = actionName.StartsWith("http", StringComparison.CurrentCultureIgnoreCase) ? actionName : Url.Action(actionName, controlName);
            return Content(string.Format("<script>alert('{0}');{1}window.location.href='", msg, isTop ? "top." : "") + url + "'</script>");
        }
        /// <summary>
        /// 跳转页带提示信息(同步)
        /// </summary>
