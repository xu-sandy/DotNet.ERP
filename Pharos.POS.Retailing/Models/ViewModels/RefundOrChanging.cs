            ...
            set
            {
                ...
                if (!string.IsNullOrEmpty(barcode))
                {
                    if (OldOrderList != null && OldOrderList.Count() > 0)
                    {
                        ...
                        {
                            var hasbarcode = OldOrderList.Any(o => o.Barcode == barcode);
                            if (!hasbarcode)
                            {
                                CurrentWindow.Dispatcher.Invoke(new Action(() =>
                                {
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        CurrentWindow.Dispatcher.Invoke(new Action(() =>
                        {
                            if (resultAuth.Code == "200")
                            {
                                    ...
                                    if (o == ConfirmMode.Confirmed)
                                    {
                                        ...
                                        CurrentWindow.Dispatcher.Invoke(new Action(() =>
                                        {
                                            Task.Factory.StartNew(() =>
                                            {
                                                ...
                                                {
                                                    //打开支付界面
                                                    MultiPay page = new MultiPay(Difference, (Status == ChangeStatus.Change ? PosModels.PayAction.Change : PayAction.Refund));
                                ...
                                //        StoreId = _machinesInfo.StoreId,
                                //        MachineSn = _machinesInfo.MachineSn,
                                //        CompanyId = _machinesInfo.CompanyId,
                                //        Mode = Status
                                //    };
